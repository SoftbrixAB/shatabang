...
import DirectoryList from '../modules/directory_list';
...
interface TaskQueue {
  ...
  queueTask(taskName: string, data?: any, priority?: string | number): Promise<any>;
  clearQueue(taskName: string): Promise<void>;
  retryFailed(): void;
}
...
const init = function(config: Config, task_queue: TaskQueue): void {
  ...
  task_queue.registerTaskProcessor('upgrade_check', function(data, job, done) {
    ...
    redis.get(versionKey, async function(err: Error | null, version: string | null) {
      if (err) {
...
const upgrade_v1 = function(infoDirectory: string, storageDir: string, cb: (error?: any) => void): void {
  shFiles.listSubDirs(infoDirectory, function(error: Error | null, dirs: string[]) {
    if (error) {
      return cb(error);
    }
...
async function import_meta_to_index(infoDirectory: string, storageDir: string, task_queue: TaskQueue): Promise<void> {
...
async function move_v_tmp_files_to_cache(infoDirectory: string, storageDir: string, cacheDir: string): Promise<void> {
  const items = await allMedia(infoDirectory);
  let cnt = 0;
  for (const i in items) {
    const relativeDest = items[i];
...
async function add_import_cache(infoDirectory: string, storageDir: string, cacheDir: string): Promise<void> {
  const importLog = new ImportLog(cacheDir);
  ...
  const items = await allMedia(infoDirectory);
  const datesTimes = new Set<number>();
  for (const i in items) {
    const relativeDest = items[i];
...
async function addImageSize(infoDirectory: string, task_queue: TaskQueue, size: any): Promise<void> {
  const items = await allMedia(infoDirectory);
  for (const i in items) {
    const relativeDest = items[i];
    const data = {
...
async function reecode_videos(infoDirectory: string, storageDir: string, cacheDir: string, task_queue: TaskQueue): Promise<void> {
...
async function updateMediaLists(storageDir: string, cacheDir: string): Promise<void> {
  const dirs = await shFiles.listSubDirsAsync(storageDir);

  return Promise.all(dirs.map((dir: string) => {
    if (!isNumber(dir)) {
...
async function allMedia(infoDirectory: string): Promise<string[]> {
...
async function getTimestamp(relativeDest: string, storageDir: string): Promise<number | undefined> {
